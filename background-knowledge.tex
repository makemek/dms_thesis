...
These two metadata refer to only this \enquote{Julia Roberts} \includegraphics[scale=0.3]{res/bg-knowledge/metadata-julia} and not any other Julia Roberts. 
Same with a second example, A branch in \enquote{111 Wall Street} with a branch manager \enquote{Sam Sneed} refer only to this \includegraphics[scale=0.3]{res/bg-knowledge/metadata-wallstreet} branch.
...
\begin{description}
	...
	They don't store record's relation, but rather graph database with key-value pairs.
...
For example, a simple customer's receipt with orders and billing addresses from figure \ref{nosql-aggregate-uml} can be modelled to hierarchical parent-child relationship in figure \ref{nosql-aggregate-uml-realized}. 
